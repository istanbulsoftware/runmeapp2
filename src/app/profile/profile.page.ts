...
import { AdvertPrivatePage } from '../advert-private/advert-private.page';
...
export class ProfilePage implements OnInit {
  ...
  constructor(
    ...
    this.acticeRoute.params.pipe(take(1)).subscribe(params => {
      this.id = params.id
  ...
  createMessagereq(){
    this.http.post(`${environment.url}/save-mes-req`, {  otherId: this.id }).subscribe(
      (res: any) => {
        console.log(res);
          this.decCoins(10)
      },
  ...
  onSendMessage() {
    if (this.myId == this.id) {
       this.presentToast(this.trans.instant('PROFILE.noMyProfileSend'))
    } else {      
  ...
  async presentActionSheet() {
    const actionSheet2 = await this.actionSheetController.create({
      buttons: [
        {
          text: "Uygunsuz içerik bildir",
          icon: 'alert-circle-outline',
          handler: () => {
            if (this.blockedBefore) { this.presentToast('Daha önce engellenmiş ve şikayet dilmiş') }
            else if (!this.blockedBefore) {
              this.http.post(`${environment.url}/block-new-user`, { user1: this.myId, user2: this.id }).subscribe(
                (res: any) => { this.presentToast("Başarılı bir şekilde yapıldı..."); this.checkBlock() },
                (err) => { this.presentToast("Hata oluştu lütfen tekrar deneyin") }
              )
            }
          }
